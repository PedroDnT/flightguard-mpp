...
import {
  ...
  buildTempoChain,
  PayoutEngine,
} from "../src/integrations/payout.js";
...
describe("buildPayoutMemo", () => {
  it("formats as flightguard:{policyId}:{flightNumber}:{date}", () => {
    expect(buildPayoutMemo("p-1", "AA100", "2026-03-24")).toBe(
      "flightguard:p-1:AA100:2026-03-24",
    );
  });
  ...
  it("preserves values without transformation", () => {
    expect(buildPayoutMemo("abc", "FR999", "2026-12-31")).toBe(
      "flightguard:abc:FR999:2026-12-31",
    );
  });
});
