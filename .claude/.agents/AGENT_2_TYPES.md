# AGENT 2 — TypeScript Types, Interfaces & Constants

## Your Role
Write `src/types.ts`. This is the single source of truth for all types used across the project. Every other agent imports from here. Do not write logic — only types, interfaces, enums, and constants.
...
Expected: zero errors (types only, nothing to resolve yet).

---

## Completion Checklist

- [ ] `src/types.ts` written with all interfaces above
- [ ] No logic, no imports from other src files
- [ ] `npx tsc --noEmit` passes

## Hand-off to Agents 3, 4, 5

All three can now run in parallel — they all import from `src/types.ts`.
